...
class IntArray():
    ...
    def remove(self, index:int) -> int:
        ...
        #modify the size of original list 
        self._size -= 1
        
        #create new_resmem to copy the value from original resmem
        new_resmem = ReservedMemory((self._size) * self._bytes_per_element)
        
        #copy the value from index 0 to the index which before the inputted index
        new_resmem.copy(self._resmem, count=index*self._bytes_per_element )
        
        #copy value from the index after the removed index to the last index
        new_resmem.copy(self._resmem, count=(self._size - index )*self._bytes_per_element, source_index=(index+1) * self._bytes_per_element, destination_index=index *self._bytes_per_element)
        
        #assign new_resmem to original resmem
        self._resmem = new_resmem
